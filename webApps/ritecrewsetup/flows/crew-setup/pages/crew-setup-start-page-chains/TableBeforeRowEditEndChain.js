...
define([
  ...
  class TableBeforeRowEditEnd extends ActionChain {
    ...
    async run(context, { cancelEdit, rowKey, rowIndex, rowData, currentRowBuffer, event }) {
      ...
      if (!$page.variables.disableRowEditExit && $page.functions.isFormValid(event)) {
        ...
        if (cancelEdit) {
          ...
          if (rowHasChanged) {
            await $page.variables.crewTemplateBDP.instance.updateItem({ metadata: { key: rowKey}, data: currentRowBuffer});
     ...
     * @param {getCrewTemplateLines} params.oldRow 
     * @param {getCrewTemplateLines} params.newRow 
     */
    async areDifferent(context, { key, newRow, oldRow }) {
      ...
      if (!originalValue) {
        ...
        if (same) {
          ...
          $page.variables.crewTemplateBDP.instance.resetUnsubmittedItem(key);
          return false;
        }
      }
